# 飞书群机器人 - 今天吃什么

一个飞书群机器人，当在群里@机器人时，会随机返回一个餐厅推荐。

## 功能特性

- 🤖 响应群聊中的@消息
- 🎲 随机选择餐厅
- 🍽️ 支持45个候选餐厅
- ✅ 自动处理飞书事件回调

## 快速开始

### 1. 安装依赖

```bash
npm install
```

### 2. 配置飞书应用

#### 2.1 创建飞书应用

1. 访问 [飞书开放平台](https://open.feishu.cn/)
2. 创建企业自建应用
3. 记录下 `App ID` 和 `App Secret`

#### 2.2 配置权限

在应用管理页面，需要开启以下权限：
- `im:message` - 获取与发送单聊、群组消息
- `im:message:group_at_msg:readonly` - 接收群聊中@机器人消息事件

#### 2.3 配置事件订阅

1. 进入"事件订阅"页面
2. 添加事件：`im.message.receive_v1` (接收消息)
3. 设置请求地址：`https://your-domain.com/webhook`
4. 如果配置了加密，需要填写 `Encrypt Key`
5. 如果配置了验证令牌，需要填写 `Verification Token`

#### 2.4 添加机器人到群聊

1. 在应用管理页面，进入"机器人"功能
2. 将机器人添加到目标群聊

### 3. 配置环境变量

复制 `.env.example` 为 `.env` 并填写配置：

```bash
cp .env.example .env
```

编辑 `.env` 文件：

```env
FEISHU_APP_ID=your_app_id_here
FEISHU_APP_SECRET=your_app_secret_here
FEISHU_ENCRYPT_KEY=your_encrypt_key_here  # 可选
FEISHU_VERIFICATION_TOKEN=your_verification_token_here  # 可选
PORT=3000
```

### 4. 启动服务器

```bash
npm start
```

或者使用开发模式（自动重启）：

```bash
npm run dev
```

### 5. 部署

#### 使用 ngrok 进行本地测试

```bash
ngrok http 3000
```

将 ngrok 提供的 https 地址配置到飞书应用的事件订阅中。

#### 生产环境部署

本项目支持多种部署平台，推荐使用以下平台：

##### 方式一：Railway（推荐）

1. 访问 [Railway](https://railway.app/)
2. 使用 GitHub 账号登录
3. 点击 "New Project" > "Deploy from GitHub repo"
4. 选择你的仓库
5. 在项目设置中添加环境变量：
   - `FEISHU_APP_ID`
   - `FEISHU_APP_SECRET`
   - `FEISHU_ENCRYPT_KEY` (可选)
   - `FEISHU_VERIFICATION_TOKEN` (可选)
6. Railway 会自动部署，获取生成的域名
7. Webhook 地址：`https://your-app.railway.app/webhook`

##### 方式二：Render

1. 访问 [Render](https://render.com/)
2. 使用 GitHub 账号登录
3. 点击 "New" > "Web Service"
4. 连接你的 GitHub 仓库
5. 配置：
   - Build Command: `npm install`
   - Start Command: `npm start`
6. 添加环境变量（同 Railway）
7. 点击 "Create Web Service"
8. Webhook 地址：`https://your-app.onrender.com/webhook`

##### 方式三：Fly.io

1. 安装 Fly CLI: `curl -L https://fly.io/install.sh | sh`
2. 登录: `fly auth login`
3. 部署: `fly launch`
4. 设置密钥: `fly secrets set FEISHU_APP_ID=xxx FEISHU_APP_SECRET=xxx`
5. Webhook 地址：`https://your-app.fly.dev/webhook`

##### 方式四：Docker 部署

```bash
# 构建镜像
docker build -t feishu-bot-what-to-eat .

# 运行容器
docker run -d \
  -p 3000:3000 \
  -e FEISHU_APP_ID=your_app_id \
  -e FEISHU_APP_SECRET=your_app_secret \
  --name feishu-bot \
  feishu-bot-what-to-eat
```

##### 方式五：传统服务器部署

1. 在服务器上安装 Node.js 18+
2. 克隆项目: `git clone your-repo-url`
3. 安装依赖: `npm install`
4. 配置环境变量
5. 使用 PM2 运行: `pm2 start server.js --name feishu-bot`
6. 配置 Nginx 反向代理（可选）

**部署后获取 Webhook 地址：**

部署完成后，你的 Webhook 地址格式为：
- Railway: `https://your-app.railway.app/webhook`
- Render: `https://your-app.onrender.com/webhook`
- Fly.io: `https://your-app.fly.dev/webhook`
- 自定义域名: `https://your-domain.com/webhook`

将此地址配置到飞书应用的事件订阅中。

## 使用方法

在飞书群聊中，@机器人或发送包含@机器人的消息，机器人会自动回复一个随机餐厅。

例如：
```
@今天吃什么
```

机器人会回复：
```
🎲 今天去吃：**寿司沼津港漕河泾印象城店** 🍽️
```

## 自定义餐厅列表

编辑 `restaurants.json` 文件，修改餐厅列表：

```json
[
  "餐厅1",
  "餐厅2",
  "餐厅3"
]
```

修改后重启服务器即可生效。

## 项目结构

```
feishu_bot_what_to_eat/
├── server.js           # 主服务器文件
├── restaurants.json    # 餐厅列表
├── package.json        # 项目配置
├── .env.example        # 环境变量示例
└── README.md          # 说明文档
```

## 注意事项

1. 确保服务器可以访问飞书开放平台 API（需要外网访问）
2. 如果使用加密，需要正确配置 `FEISHU_ENCRYPT_KEY`
3. 机器人需要被添加到群聊中才能接收消息
4. 确保应用有足够的权限

## 故障排查

### 机器人不响应消息

1. 检查应用权限是否已开启
2. 检查事件订阅是否正确配置
3. 检查服务器日志是否有错误
4. 确认机器人已添加到群聊

### Token 获取失败

1. 检查 `APP_ID` 和 `APP_SECRET` 是否正确
2. 检查网络连接是否正常
3. 查看服务器日志错误信息

## License

MIT

